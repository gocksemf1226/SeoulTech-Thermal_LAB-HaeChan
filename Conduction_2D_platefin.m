...
function Conduction_2D_platefin(nelx,nely,~,~,~);
% INITIALIZE
x(1:nely,1:nelx) = 0.;
w = setdiff([1:nely],[1:4:nely]);
for j = w
    for i = 1:nelx
        x(j,i) = 1;
    end
...
%   change = max(max(abs(x-xold)));
  disp([' Vol.: ' sprintf('%6.3f',sum(sum(x))/(nelx*nely)) ...
...
for elx = 1:nelx
  for ely = 1:nely
    ...
    K(edof,edof) = K(edof,edof) + (0.001+0.999*x(ely,elx))*KE;
  end    
end
