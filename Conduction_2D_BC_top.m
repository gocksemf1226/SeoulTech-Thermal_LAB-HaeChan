...
% Conduction_2D_HotSpot_top(nelx,nely,volfrac,penal,rmin)

function Conduction_2D_BC_top(nelx,nely,volfrac,penal,rmin);
% INITIALIZE
...
%   top_plot = subplot(211); colormap(top_plot, gray); %HC
  ...
  fig1 = figure(1); %HC
  set(fig1, 'OuterPosition', [0, 100, 1000,1000]) %HC
%   mean_temp_plot = subplot(212); colormap(mean_temp_plot, jet); %HC
  colormap(jet); imagesc(Ue_mean); colorbar; axis equal; axis tight; axis off; pause(1e-2); %HC
  fig2 = figure(2); %HC
  set(fig2, 'OuterPosition', [1000, 100, 1000,1000]) %HC
end
figure(3); %HC
fig3 = figure(3); %HC
set(fig3, 'OuterPosition', [400, 0, 1000, 800]); %HC
